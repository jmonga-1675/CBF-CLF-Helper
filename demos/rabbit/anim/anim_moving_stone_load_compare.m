function anim_moving_stone_load_compare(nonrobust_data,robust_data)
% animation to compare nonrobust and robust CBF-QP 
% with time varying stepping stone while carrying unknown load
% 04/05/2016
ts=1/30;
%% load data for nonrobust control
load(nonrobust_data);
[n,m] = size(x);
n_full=n;
l_min=l_min_t(1:n);
...
[te,pH_horiz2]=even_sample(t,pH_horiz,1/ts);
[te2,xe2]=even_sample(t,x,1/ts);
[n,m]=size(xe2);

%% load data for robust control
load(robust_data);
[n,m] = size(x);
n_full=n;
l_min=l_min_t(1:n);
...
for k = 1:n
% draw robot with nonrobust ctrl
    % dim out robot color
    cr=[1 0 0]; cb=[0 0 1]; cg=[0 1 0];
    gray=0.7*[1 1 1]; alpha=0.3;
    color{1}=cr*alpha+gray*(1-alpha);
    color{2}=cg*alpha+gray*(1-alpha);
    color{3}=cb*alpha+gray*(1-alpha);
    
    % animate how robot fall
    k2=length(te2);
    if k<=k2
        q2=xe2(k,1:5);
          out2 = limb_position(q2,pH_horiz2(k));
        drawone_robot_only(axes1,out2,color);
    elseif k <=k2+10
            q2=xe2(k2,1:5)*0.0001*(k-k2);
          out2 = limb_position(q2,pH_horiz2(k2));
        drawone_robot_only(axes1,out2,color);
    else
        q2=xe2(k2,1:5)*0.0001*10;
          out2 = limb_position(q2,pH_horiz2(k2));
        drawone_robot_only(axes1,out2,color);
    end
    ...
    % display ctrl name
    xt=out2.pH1-1.2;
    yt=figure_y_limits(2)-0.75;
    disp_ctrl2=text(xt,yt,'Non-Robust');
    set(disp_ctrl2,'FontSize',14);
    set(disp_ctrl2,'color',0.4*[1 1 1]);
    
% draw robot with robust ctrl
...
% joints
    ...
    xt=(figure_x_limits(1)+figure_x_limits(2))/2+out.pH1+0.9;
    yt=figure_y_limits(2)-0.4;
    disp_mass=text(xt,yt,['m_{load}=',num2str(m_load),' [kg](47%)']);
    set(disp_mass,'FontSize',14);
  
    % Display ctrl name
    xt=out.pH1+0.5;
    yt=figure_y_limits(2)-0.75;
    disp_ctrl1=text(xt,yt,'Robust');
    set(disp_ctrl1,'FontSize',14);
    
    % update axes to keep hip position fixed
    set(parent,'XLim',figure_x_limits+[out.pH1 out.pH1],'YLim',figure_y_limits);
end

function drawone_robot_only(parent, out, robot_color)
% delete previous screen
tem = get(parent,'Children');
delete(tem);
    
leg1_color=robot_color{1};
leg2_color=robot_color{2};
torso_color=robot_color{3}; 

...
C = inv(phi'*phi)*phi'*[th1(1); th1(nt)];
u = 1/C(1)*(th1-C(2));
%plot(th1,u)
...
for WHICH_ONE = 1:4
  ...
  [a_final_tmp,f] = fminunc('bezier_obj', a, options);
  
  % impose symmetry
  switch WHICH_ONE
   case 0
    a_final(WHICH_ONE,:) = [traj(1) a_final_tmp traj(1)]; % changed by grizzle to remove symmetry on torso
   case {1,2,3,4}
    a_final(WHICH_ONE,:) = [traj(1) a_final_tmp traj(nt)]; 
  end
  ...
  if 1
    ...
    switch WHICH_ONE
     case 1
      title('Torso angle (done)')
      ylabel('q_1');
     case 2
      title('Angle between femurs (done)')
      ylabel('q_{32}-q_{31}');
     case 3

...
if ~isfield(settings, 'event_func')
    event_func = [];
else
    event_func = settings.event_func;
end
...
for k = 1:total_k-1
    ...
    if k ==1 && ~isempty(mu0)
        ...
        [mu, extra_t] = controller(x, zeros(control_sys.udim, 1), with_slack, verbose); % mu
    ...
    if isempty(event_func)
        ...
        event_options = odeset('Events', event_func);
        [ts_k, xs_k, te, xe, ~] = ...
